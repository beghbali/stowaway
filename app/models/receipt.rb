class Receipt < ActiveRecord::Base
  ...
  scope :for, ->(ride) { geocoded.where(billed_to: ride.captain.rider.email).where(ride_requested_at: (ride.captain.checkedin_at - REQUEST_TIME_PROXIMITY)..(ride.captain.checkedin_at + REQUEST_TIME_PROXIMITY)).near(ride.pickup_location)}
